...
<c:forEach items="${recommendPatents}"  var="patents">


<div class="floor_1F">
	<c:forEach items="${secondColumns}" var="secondColumn">
		<c:if test="${secondColumn.id==patents.key }">
			<div class="news-info-hd" style="clear:both;height: 50px;border-bottom: 2px solid #68b4ef;">
			        <h2 style="font-size: 24px;color: #333;line-height: 25px;">${secondColumn.name}</h2>                           
			</div>
		</c:if>
	</c:forEach>
